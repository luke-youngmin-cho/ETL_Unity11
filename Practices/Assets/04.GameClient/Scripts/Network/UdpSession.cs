...
namespace Practices.GameClient.Network
{
    ...
    public abstract class UdpSession
    {
        ...
        /// 송신 대기열에 송신할 데이터를 등록
        /// </summary>
        /// <param name="message"> 송신할 데이터 </param>
        /// <param name="remoteEndPoint"> 데이터를 받아야할 이용자(클라이언트) </param>
        protected void InternalSend(IMessage message, EndPoint remoteEndPoint)
        {
            ...
            try
            {
                ...
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
                    ...
                    DebugLogger.Log($"[{nameof(UdpSession)}] send to {Socket.RemoteEndPoint}. total {bytesWritten} bytes.");
                }
            }
            catch (Exception ex)
            {
            ...
            while (IsRunning)
            {
                try
                {
                    ...
                    while (SendQueue.TryDequeue(out (ArraySegment<byte> segment, EndPoint remoteEndPoint) segmentPair))
                    {
                        try
                        {
                            int bytesSent = await Socket.SendToAsync(segmentPair.segment, SocketFlags.None, segmentPair.remoteEndPoint); // 데이터 송신
                            DebugLogger.Log($"[{nameof(UdpSession)}] Sent data length of {bytesSent}.");
                        }
                        catch (Exception ex)
                        {
                            DebugLogger.Log($"[{nameof(UdpSession)}] Failed to send data.");
                        }
                    }
                }
                catch (Exception ex)
        ...
        async Task ReceiveLoopAsync()
        {
            ...
            while (IsRunning)
            {
                ...
                try
                {
                    SocketReceiveFromResult result = await Socket.ReceiveFromAsync(new ArraySegment<byte>(buffer), SocketFlags.None, senderEndPoint); // 누구든지 UDP 로 데이터를 나한테 보낸 클라이언트가 있을때까지 기다림
                    ...
                    {
                        DebugLogger.Log($"[{nameof(UdpSession)}] Received invalid data.");
                    ...
                    {
                        MessageType messageType = (MessageType)reader.ReadUInt16(); // 젤앞에 ushort 는 messageType 으로 송신되었을것이므로 수신시에도 동일하게 처리
                        IMessage message = MessageFactory.Create(messageType);

                        // 길이는 올바르지만 messageType 이 잘못 들어왔을수도 있으므로(message 객체만드는데 실패했을것임) 확인
                        if (message != null)
                        {
                            message.Deserialize(reader); // message 객체를 수신받은 데이터로 초기화
                            HandleMessage(message, senderEndPoint); // message 를 취급하는 구현부에서 작성된 내용대로 처리할것.
                        }
                        else
                        {
                            DebugLogger.Log($"[{nameof(UdpSession)}] Received invalid message type.");
                        }
                    }
        ...
        protected abstract void HandleMessage(IMessage message, EndPoint senderEndPoint);
    }
}
