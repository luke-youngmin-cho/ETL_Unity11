            ...
            try
            {
                ...
                lock (SendLock)
                {
                    while (offset < entireLength)
                    {
                        ...
                        byte[] chunk = BufferPool.Rent(chunkSize); // BufferPool 에서 가져올때는 정확하게 파라미터사이즈 크기의 배열을 주는게 아니라 그 크기 이상 인 버퍼를 찾아서 줌.
                        Buffer.BlockCopy(entireData, offset, chunk, 0, chunkSize);

                        ArraySegment<byte> segment = new ArraySegment<byte>(chunk, 0, 48);
